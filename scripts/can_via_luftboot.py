...
if __name__ == "__main__":
	...
	try:
		...
		print "No CAN address given."
		raise

	if can_address > 0x7ff:
		print "Invalid CAN11 address."
		(exit -1)

	addr = can_address*64*1024


	while bin:
		print ("Programming memory at 0x%08X\r" % (addr/can_address)),
		stdout.flush()
		...
		bin = bin[SECTOR_SIZE:]
		addr += SECTOR_SIZE
		if addr > address*65*1024:
			print "BIN too long >64KBytes."
			(exit -1)
