...
static bool
gw_can_sendrec(can_msg_t *tx_msg, can_msg_t *rx_msg_ref, uint32_t timeout,
bool check_id, bool check_len, uint8_t check_data_map);
static bool
gw_can_sendrec_w(can_msg_t *tx_msg, can_msg_t *rx_msg_ref, uint32_t timeout,
bool check_id, bool check_len, uint8_t check_data_map);
bool
gw_can_erase_sector(uint32_t address);
static void
gw_can_init(uint32_t baud);
bool
...
void gw_can_init(uint32_t baud)
{
    ...
    CAN_RIxR_STID_MASK << CAN_RIxR_STID_SHIFT, 0, true);
...
bool gw_can_erase_sector(uint32_t address)
{
    ...
    *(uint32_t *) (&rx_frame.data[0]) = 0x00502060;
    ...
    if (!gw_can_sendrec_w(&tx_frame, &rx_frame, NODE_ERASE_TIMEOUT, false, true,
                    0b00000111))
    {
